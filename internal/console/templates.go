...
<body>
  ...
  <main class="page">
    ...
    {{if .Projects}}<form class="panel" method="post" action="/dispatch" autocomplete="off">
      ...
      <div class="inputs-heading"><div><h2>Inputs</h2><span class="hint" id="inputs-hint">{{if .InputsFromWorkflow}}Inputs declared by this workflow file.{{else}}Load the workflow to see its inputs.{{end}}</span></div></div>
      <div id="inputs">{{if .InputsFromWorkflow}}{{range $index, $input := .Inputs}}<div class="input-row">
      ...
      </div>{{else}}<div class="hint">This workflow does not declare any inputs.</div>{{end}}{{end}}</div>
      <button class="button submit" id="run-workflow" type="submit"{{if not .InputsFromWorkflow}} disabled{{end}}>Run workflow</button>
    </form>{{else}}<p class="notice">No GitHub Projects are configured.</p>{{end}}
  </main>
  <script>
    const inputs=document.getElementById('inputs');
    for(const field of document.querySelectorAll('#workflow-selection input, #workflow-selection select')){
      const invalidate=()=>{
        document.getElementById('loaded-selection').value='';
        document.getElementById('run-workflow').disabled=true;
        inputs.replaceChildren();
        document.getElementById('inputs-hint').textContent='Load the workflow to see its inputs.';
      };
...
<body data-stream-url="{{.StreamURL}}">
  ...
  <script>
    const output=document.getElementById('logs');const placeholder=document.getElementById('placeholder');const state=document.getElementById('state');const statePill=document.getElementById('state-pill');const timeToggle=document.getElementById('time-toggle');
    const clusterSize=50;const renderEntryLimit=500;const maxRenderedClusters=80;const overscan=1200;const maxRenderedLineCharacters=4096;
    ...
    let lineNumber=0;let received=false;let queuedEntries=[];let queueIndex=0;let renderScheduled=false;
    ...
    const conclusionLabel=value=>({success:'Succeeded',failure:'Failed',cancelled:'Cancelled',skipped:'Skipped'})[value]||'Running';
    ...
    function logPreview(value){const text=value||'';if(text.length<=maxRenderedLineCharacters)return text;let end=maxRenderedLineCharacters;const finalCode=text.charCodeAt(end-1);if(finalCode>=0xd800&&finalCode<=0xdbff)end--;return text.slice(0,end)}
    function appendLogPart(target,part,text){const span=document.createElement('span');span.textContent=text;if(part.foreground)span.style.color=part.foreground;if(part.background)span.style.backgroundColor=part.background;if(part.bold)span.style.fontWeight='700';if(part.dim)span.style.opacity='.65';if(part.italic)span.style.fontStyle='italic';const decoration=[];if(part.underline)decoration.push('underline');if(part.strike)decoration.push('line-through');if(decoration.length)span.style.textDecorationLine=decoration.join(' ');target.append(span)}
    function appendLogText(target,entry,preview){
      const value=preview?logPreview(entry.text):entry.text||'';if(!entry.parts){target.append(document.createTextNode(value));return}
      let remaining=value.length;for(const part of entry.parts){if(remaining===0)break;const text=(part.text||'').slice(0,remaining);appendLogPart(target,part,text);remaining-=text.length}
    }
    function renderLogText(target,record){
      target.replaceChildren();const value=record.entry.text||'';if(value.length<=maxRenderedLineCharacters){appendLogText(target,record.entry,false);return}
      const button=document.createElement('button');button.type='button';button.className='line-expand';button.textContent=record.expanded?'Collapse line':'Show full line ('+(value.length-logPreview(value).length).toLocaleString()+' more characters)';button.addEventListener('click',()=>{record.expanded=!record.expanded;renderLogText(target,record)});
      if(record.expanded){target.append(button,document.createElement('br'));appendLogText(target,record.entry,false)}else{appendLogText(target,record.entry,true);target.append(button)}
    }
    function createLine(record){
      ...
      const text=document.createElement('span');text.className='line-text';const content=document.createElement('span');text.append(content);
      if(entry.kind==='input'||entry.kind==='step-output'){const key=document.createElement('span');key.className='metadata-key';key.textContent=(entry.kind==='input'?'with ':'output ')+(entry.text||'');content.append(key)}else{renderLogText(content,record)}
    ...
    function estimateClusterHeight(cluster){
      ...
      for(const record of cluster.records){const entry=record.entry;let text=record.expanded?entry.text||'':logPreview(entry.text);if(entry.kind==='input'||entry.kind==='step-output')text=(entry.kind==='input'?'with ':'output ')+text;let rows=0;for(const part of text.split('\n'))rows+=Math.max(1,Math.ceil(Math.max(1,part.length)/width));if(entry.properties&&(entry.properties.title||entry.properties.file||entry.properties.line))rows++;height+=Math.max(20,rows*19.8)}
      return height;
    }
    ...
    function addEntry(entry,appender,renderNewClusters,groupsToCollapse){
      ...
      const record={entry,number:++lineNumber,expanded:false};cluster.records.push(record);if(cluster.rendered)appender.append(cluster.element,createLine(record));else updateClusterEstimate(cluster);
    }
    function refreshVirtualEstimates(){for(const cluster of clusters)updateClusterEstimate(cluster)}
    function flushEntries(){
      renderScheduled=false;if(queueIndex===queuedEntries.length)return;const pinned=window.innerHeight+window.scrollY>=document.documentElement.scrollHeight-80;const appender=createAppender();const groupsToCollapse=[];const end=Math.min(queueIndex+renderEntryLimit,queuedEntries.length);
      ...
      trimRenderedClusters();if(pinned)window.scrollTo(0,document.documentElement.scrollHeight);
    ...
    timeToggle.addEventListener('change',()=>{output.classList.toggle('show-time',timeToggle.checked);requestAnimationFrame(refreshVirtualEstimates)});

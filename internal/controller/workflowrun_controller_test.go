...
func TestWorkflowRunLabelsUseLineageAndGitHubStatusIdentity(t *testing.T) {
	...
	projectMatches, err := reporter.ensureGitHubStatusIdentityLabels(context.Background(), stored, "project-uid", statusKey)
	if err != nil {
		t.Fatal(err)
	}
	if !projectMatches {
	...
	projectMatches, err = reporter.ensureGitHubStatusIdentityLabels(context.Background(), stored, "recreated-project-uid", statusKey)
...
func TestCanceledWorkflowRunWaitsForPodsBeforeFinalizing(t *testing.T) {
	...
	if _, err := reporter.Reconcile(context.Background(), ctrl.Request{NamespacedName: client.ObjectKeyFromObject(run)}); err != nil {
		t.Fatal(err)
	}
	stored = &actionsv1alpha1.WorkflowRun{}
	if err := clusterClient.Get(context.Background(), client.ObjectKeyFromObject(run), stored); err != nil {
